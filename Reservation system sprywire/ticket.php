...
<html lang="en">
<head>
<title>kisumu Internatinal Airport:TICKET</title>
<meta charset="utf-8">
...
<body>
  <center>
    ...
    <h1 style="float:left;">kisumu Internatinal Airport</h1>
    <p style="float:right;padding:1%;">Toll free: 0717618988</p>
    </div>
    <hr style="border-top:2px solid #0086b3;">
    <div>
      <img src="logo.png" alt="kisumu Internatinal Airport" style="float:left;width:15%">
<?php
$my_bid=$_SESSION['bid'];
$my_uid=$_SESSION['userid'];
$my_fid=$_SESSION['fid_chosen'];
/*AUTO-INCREMENT FEATURE HAS BEEN ADDED FOR TICKET ID*/
//Inserting values into TICKET table
$sql="INSERT INTO TICKET (BID,TAX,T_DATE,T_TIME) VALUES('".$my_bid."',5,current_date,current_time)";
if($conn->query($sql) === TRUE){
  $latest_id=$conn->insert_id;
}
else{
  echo "ERROR RECORD ID";
}

//Updating the airfare and total_airfare of TICKET table
$sql="UPDATE TICKET SET AIRFARE = (SELECT PRICE_PER_PERSON * NOP FROM FLIGHT INNER JOIN BOOKING ON FLIGHT.FID=BOOKING.FID WHERE BOOKING.BID='".$my_bid."') WHERE TID= '".$latest_id."'";
if($conn->query($sql) === TRUE){
}
else{
  echo "ERROR RECORD AIRFARE";
}
$sql="UPDATE TICKET SET TOTAL_AIRFARE= 1.05*AIRFARE WHERE TID= '".$latest_id."'";
if($conn->query($sql) === TRUE){
}
else{
  echo "ERROR UPDATE TOTAL_AIRFARE";
}
...
if ($result->num_rows > 0) {
 ...
 <a href="logout.php"><button style="background-color:#0086b3;color:white;">Logout</button></a>
